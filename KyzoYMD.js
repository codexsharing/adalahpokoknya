...
module.exports = KyzoYMD = async (KyzoYMD, m, chatUpdate, store) => {
    try {
...
const sender = m.key.fromMe ? KyzoYMD.user.id.split(":")[0] + "@s.whatsapp.net" || KyzoYMD.user.id
...
const botNumber = await KyzoYMD.decodeJid(KyzoYMD.user.id);  
...
const KyzoYMDdev = (m && m.sender && [botNumber, ...global.owner].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)) || false;  
...
const groupMetadata = m?.isGroup ? await KyzoYMD.groupMetadata(m.chat).catch(() => ({})) : {};  
...
const handleData = { KyzoYMD, text, args, isOwn, isPrem, isCmd, command, reply, conn, sock, quoted, fetchJson, randomKarakter, formatSize, sleep, smsg, isOwner, isPremium, isCmd, prefix, usedPrefix }
...
// MAIN FUNCTION
function totalFitur() {
  const CASE = countCase("./KyzoYMD.js");
...
default: 
        ...
        if (m && m.chat) {
            try {
                await KyzoYMD.sendMessage("0@s.whatsapp.net", {
